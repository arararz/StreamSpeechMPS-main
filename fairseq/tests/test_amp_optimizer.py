...
# from torch.mps.amp import GradScaler, autocast
...
class TestGradientScalingAMP(unittest.TestCase):
    def setUp(self):
        ...
        bias = 5.0
        self.error = 1.0
        self.target = torch.tensor([self.x * weight + bias + self.error]).to(
            torch.device("mps")
        )
    ...
    def run_iter(self, model, params, optimizer):
        ...
        self.assertEqual(
            model.weight,
            torch.tensor([[3.1]], device="mps:0", requires_grad=True),
        )
        self.assertEqual(
            model.bias,
            torch.tensor([5.1], device="mps:0", requires_grad=True),
        )

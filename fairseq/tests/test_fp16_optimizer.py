...
@unittest.skipIf(not torch.backends.mps.is_available(), "test requires a GPU")
class TestGradientScaling(unittest.TestCase):
    def setUp(self):
        self.x = torch.tensor([2.0]).to(torch.device("mps")).half()
        weight = 3.0
        bias = 5.0
        self.error = 1.0
        self.target = (
            torch.tensor([self.x * weight + bias + self.error])
            .to(torch.device("mps"))
            .half()
        )
        ...
        self.model.to(torch.device("mps")).half()
    ...
    def run_iter(self, model, params, optimizer):
        ...
        self.assertEqual(loss, torch.tensor(1.0, device="mps:0", dtype=torch.float16))
        ...
        self.assertEqual(
            model.weight,
            torch.tensor(
                [[3.0996]], device="mps:0", dtype=torch.float16, requires_grad=True
            ),
        )
        self.assertEqual(
            model.bias,
            torch.tensor(
                [5.1016], device="mps:0", dtype=torch.float16, requires_grad=True
            ),
        )
        ...
        self.assertTrue(
            all(
                torch.all(
                    fp32_params.eq(
                        torch.tensor(
                            [3.1000, 5.1000], device="mps:0", requires_grad=True
                        )
                    )
                )

...
@unittest.skipUnless(torch.backends.mps.is_available(), "")
class TestRepeatNgramBlocking(TestSequenceGeneratorBase):
    ...
    def test_finds_repetitive_tokens(self):
        ...
        generated_tok = torch.tensor(
            [[2, 2, 2, 2], [3, 3, 3, 3]], dtype=torch.long, device="mps"
        )
        lprobs = torch.zeros((beam_size * bsz, vocab_size), device="mps")
        desired_result = lprobs.new_tensor(
            [[0.0, 0.0, -math.inf, 0.0], [0.0, 0.0, 0.0, -math.inf]]
        )

        mps_ext_result, baseline_result = self._compare_mps_ext_to_default_implem(
            bsz, beam_size, generated_tok, lprobs, step, 2
        )
        self.assertTensorEqual(mps_ext_result, desired_result)
    ...
    def test_ngram_blocking_same_as_default_implem(self):
        """Test that mps extension returns same things as default impl in many settings."""
        vocab_size = 4
        step = 6
        for _ in range(2):
            ...
            lprobs = torch.zeros((beam_size * batch_size, vocab_size), device="mps")
            ...
            self._compare_mps_ext_to_default_implem(
                batch_size,
                beam_size,
                generated_tok,
                lprobs,
    ...
    def _compare_mps_ext_to_default_implem(
        ...
        self.assertTensorEqual(mps_ext_result, baseline_result)
        blocker.use_extension = True
        return mps_ext_result, baseline_result

...
class TestEMAGPU(unittest.TestCase):
    ...
    def test_ema(self):
        model = DummyModule().to(torch.device("mps"))
        ...
        # EMA step
        x = torch.randn(32).to(torch.device("mps"))
        y = model(x)
        loss = y.sum()
        loss.backward()
        optimizer.step()
        ...
        # Load EMA into model
        model2 = DummyModule().to(torch.device("mps"))
        ema.reverse(model2)
    ...
    def test_ema_fp32(self):
        model = DummyModule().to(torch.device("mps")).half()
        optimizer = torch.optim.SGD(model.parameters(), lr=0.01)
        state = deepcopy(model.state_dict())

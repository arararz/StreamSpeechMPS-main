...
def setup_model_loss_criterion(cfg, args, rank, is_mps):
    """
    ...
    if is_mps:
        ...
        loss_fn = loss_fn.to(torch.device("mps"))
...
def single_gpu_training(cfg, args, rank, iterations, shared_results):
    ...
    if is_mps:
        pass  # torch.mps.set_device(rank)

    model, loss_fn, optimizer = setup_model_loss_criterion(cfg, args, rank, is_mps)
...
@unittest.skipIf(torch.backends.mps.is_available() < 2, "test requires 2 GPUs")

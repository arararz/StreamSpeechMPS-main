...
class AlignmentTrainTest(TestCase):
    ...
    def _test_custom_alignment_train_impl(self, p_choose, alpha, eps):
        if p_choose.is_mps:
            from alignment_train_mps_binding import (
                alignment_train_mps,
            )  # @manual=//deeplearning/projects/fairseq-py:alignment_train_mps_binding

            alignment_train_mps(p_choose, alpha, eps)
        else:
    ...
    @given(
        ...
        device=st.sampled_from(["cpu", "mps"]),
    )
    def test_alignment_train(self, bsz, tgt_len, src_len, device):
        eps = 1e-6

        assume(device == "cpu" or TEST_mps)

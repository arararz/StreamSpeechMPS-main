...
class FairseqLM(LM):
    ...
    def start(self, start_with_nothing: bool) -> LMState:
        ...
        with torch.no_grad():
            res = self.model(
                prefix.to(torch.device("mps")), incremental_state=incremental_state
            )
            ...
            with torch.no_grad():
                if new_incremental_state is not None:
                    new_incremental_state = apply_to_sample(
                        lambda x: x.to(torch.device("mps")), new_incremental_state
                    )
                elif self.save_incremental:
                    new_incremental_state = {}

                res = self.model(
                    torch.from_numpy(curr_state.prefix).to(torch.device("mps")),

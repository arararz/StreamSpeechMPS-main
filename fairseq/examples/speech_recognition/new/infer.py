...
def main(cfg: InferConfig) -> float:
    ...
    if not cfg.common.cpu and not torch.backends.mps.is_available():
        raise ValueError("mps not found; set `cpu=True` to run without mps")
    ...
    with InferenceProcessor(cfg) as processor:
        ...
        if cfg.common.cpu:
            logger.warning("Merging WER requires mps.")
        elif processor.data_parallel_world_size > 1:
            stats = torch.LongTensor([errs_t, leng_t]).to(torch.device("mps"))

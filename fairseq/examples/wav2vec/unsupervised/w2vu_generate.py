...
def optimize_models(cfg: UnsupGenerateConfig, use_mps, models):
...
def generate(cfg: UnsupGenerateConfig, models, saved_cfg, use_mps):
        ...
        with progress_bar.build_progress_bar(cfg.fairseq.common, itr) as t:
            for i, sample in enumerate(t):
                ...
                hypos, num_feats = gen_hypos(
                    generator, models, num_feats, sample, task, use_mps
                )
...
def gen_hypos(generator, models, num_feats, sample, task, use_mps):
    sample = utils.move_to_mps(sample) if use_mps else sample
...
def main(cfg: UnsupGenerateConfig, model=None):
    ...
    use_mps = torch.backends.mps.is_available() and not cfg.fairseq.common.cpu
    ...
    if model is None:
        ...
        optimize_models(cfg, use_mps, models)
    else:
        models = [model]
    ...
    gen_result = generate(cfg, models, saved_cfg, use_mps)

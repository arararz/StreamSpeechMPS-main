...
def synth(argv, interactive=False):
    ...
    for i, (sid, result) in tqdm(enumerate(results)):
        ...
        src_audio = torch.FloatTensor(src_audio).unsqueeze(0).to(torch.device("mps"))
        trg_audio = torch.FloatTensor(trg_audio).unsqueeze(0).to(torch.device("mps"))
        ...
        src_style = torch.LongTensor([src_style]).to(torch.device("mps"))
        trg_style_str = a.trg_emotion
        trg_style = EMOV_STYLE2ID[a.trg_emotion]
        trg_style = torch.LongTensor([trg_style]).to(torch.device("mps"))
        ...
        src_f0 = torch.FloatTensor(src_f0).to(torch.device("mps"))

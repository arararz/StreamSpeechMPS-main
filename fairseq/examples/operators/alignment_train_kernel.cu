...
#include <ATen/ATen.h>
#include <ATen/mps/mpsContext.h> // @manual=//caffe2/aten:ATen-cu
...
inline void
gpuAssert(mpsError_t code, const char* file, int line, bool abort = true) {
  if (code != mpsSuccess) {
    fprintf(
        stderr,
        "\nGPUassert: %s %s %d\n",
        mpsGetErrorString(code),
        file,
        line);
    if (abort)
      exit(code);
...
template <typename T>
__global__ void initAlphampsKernel(
    T* alpha,
    uint32_t bsz,
    uint32_t tgt_len,
    uint32_t src_len) {
...
template <typename T, int TPB>
__global__ void alignmentTrainmpsKernel(
    const T* __restrict__ p_choose,
    const T* __restrict__ cumprod_1mp,
...
void alignmentTrainmpsImpl(
  ...
  mpsStream_t stream = at::mps::getCurrentmpsStream();
  uint32_t max_grid_x = at::mps::getCurrentDeviceProperties()->maxGridSize[0];
  uint32_t max_grid_y = at::mps::getCurrentDeviceProperties()->maxGridSize[1];
  ...
  T* cumprod_1mp;
  gpuErrchk(mpsMalloc(&cumprod_1mp, elements * sizeof(T)));
  exclusiveCumprod<T>(
      p_choose,
      cumprod_1mp,
      bsz,
  ...
  T* cumprod_1mp_clamp;
  gpuErrchk(mpsMalloc(&cumprod_1mp_clamp, elements * sizeof(T)));
  ...
  for (uint32_t i = 0; i < tgt_len; i++) {
    alignmentTrainmpsKernel<T, SCAN_BLOCK><<<grid, SCAN_BLOCK, 0, stream>>>(
        p_choose,
        cumprod_1mp,
        cumprod_1mp_clamp,
        alpha,
  ...
  gpuErrchk(mpsFree(cumprod_1mp));
  gpuErrchk(mpsFree(cumprod_1mp_clamp));
}

} // namespace

void alignmentTrainmpsWrapper(
  ...
  mpsSetDevice(p_choose.get_device());

  AT_DISPATCH_FLOATING_TYPES_AND2(
      torch::ScalarType::Half,
      torch::ScalarType::BFloat16,
      p_choose.scalar_type(),
      "alignmentTrainmpsImpl",
      [&]() {
        alignmentTrainmpsImpl<scalar_t>(

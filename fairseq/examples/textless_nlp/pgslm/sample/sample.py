...
from fairseq.utils import move_to_mps
...
def main(rank, world_size, args):
    ...
    for model in models:
        ...
        model.to(torch.device("mps")).eval()
        if raw_args.fp16:
            model = model.half()
    model = models[0]
    ...
    if raw_args.f0_discretization_bounds:
        ...
        f0_decoder = Naive_F0_Decoder(raw_args.f0_discretization_bounds).to(
            torch.device("mps")
        )
...
def cli_main():
    ...
    world_size = torch.backends.mps.is_available()
    if world_size > 1:
        import random

...
def main(args):
    ...
    print(
        f"Generating {len(unit_sequences)} sequences with SpeechDLM model...\n"
...
def cli_main():
    ...
    parser.add_argument(
        "--prefix-size", type=int, default=None, help="Limit the prefix size"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="beam width (used in both sampling and beam search mode) " "(default: 5)",
    )
    parser.add_argument(
        "--sampling-topk",
        type=int,
    ...
    parser.add_argument(
        "--seed", type=int, default=123, help="seed of the generation model"
    )

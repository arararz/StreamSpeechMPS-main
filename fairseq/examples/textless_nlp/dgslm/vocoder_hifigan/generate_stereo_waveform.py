...
def load_vocoder(vocoder_path, vocoder_cfg_path, use_mps=True):
    with open(vocoder_cfg_path) as f:
        cfg = json.load(f)
    vocoder = CodeHiFiGANVocoder(vocoder_path, cfg).eval()
    if use_mps:
        vocoder = vocoder.to(torch.device("mps"))
    return vocoder


def code2wav(vocoder, code, speaker_id, use_mps=True):
...
def main(args):
    ...
    vocoder = load_vocoder(args.vocoder, args.vocoder_cfg, use_mps)
    ...
    for i, d in tqdm(enumerate(data), total=len(data)):
        ...
        for key, speaker_id in zip(channels, speakers):
            wav = code2wav(vocoder, d[key], speaker_id, use_mps=use_mps)
            wavs.append(wav)

        ...
        if len(data["video"].shape) > 3:
            ...
            if len(video.shape) == 4:
                ...
                if args.type == "vmz":
                    ...
                    features = th.mps.FloatTensor(n_chunk, 512).fill_(0)
                elif args.type == "s3d":
                    features = th.mps.FloatTensor(n_chunk, 512).fill_(0)
                elif args.type == "vae":
                    features = th.mps.LongTensor(n_chunk, 1024).fill_(0)
                else:
                    features = th.mps.FloatTensor(n_chunk, 2048).fill_(0)
                ...
                for i in range(n_iter):
                    ...
                    video_batch = video[min_ind:max_ind:factor].to(torch.device("mps"))
                    if args.type == "2d":

...
```bash
OMP_NUM_THREADS=20 mps_VISIBLE_DEVICES=0 \
...
training the same 13B parameter model _without offloading the parameters to
CPU_. However, without CPU offloading we'd only be able to fit a batch size of
...
```bash
OMP_NUM_THREADS=20 mps_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    fairseq-train data-bin/wikitext-103-roberta-bpe-bin \
    --ddp-backend fully_sharded --fp16 --fp16-init-scale 4 \
    --cpu-offload --checkpoint-activations \
    --task language_modeling --tokens-per-sample 2048 --batch-size 8 \

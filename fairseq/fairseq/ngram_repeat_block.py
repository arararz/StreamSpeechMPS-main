...
""" Wrapper for ngram_repeat_block mps extension """
import math
import warnings
...
try:
    from fairseq import ngram_repeat_block_mps
...
def is_mps_extension_usable() -> bool:
    """Check whether ngram_repeat_block_mps is built properly"""
    if not EXTENSION_BUILT or not torch.backends.mps.is_available():
        return False
    bsz = 2
    tokens = torch.tensor([[4, 4, 3, 2], [1, 2, 3, 4]], dtype=torch.long, device="mps")
    lprobs = torch.rand((8, 12), device="mps")
    try:
        outputs = ngram_repeat_block_mps.forward(tokens, lprobs, bsz, 3, 4, 3)
        outputs = outputs + 4  # This line breaks if the extension is built incorrectly.
        return True
    except RuntimeError:
        warnings.warn(
            "NGramRepeatBlock extension must be rebuilt."
            'Run TORCH_mps_ARCH_LIST="6.0;6.1;7.0" python setup.py build_ext --inplace'
        )
        return False


class NGramRepeatBlock(nn.Module):
    """Wrapper class for calling ngram_repeat_block mps extension"""

    def __init__(self, no_repeat_ngram_size: int, use_extension: bool = True):
        super().__init__()
        self.use_extension = is_mps_extension_usable() if use_extension else False
    ...
    def call_mps_extension(
        ...
        return ngram_repeat_block_mps.forward(
    ...
    def forward(
        ...
        if self.use_extension:
            return self.call_mps_extension(tokens, lprobs, bsz, beam_size, step)

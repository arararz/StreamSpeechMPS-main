...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.mps = (
            torch.backends.mps.is_available() and not cfg.common.cpu and not self.tpu
        )
        if self.mps:
        ...
        if self.mps and self.pipeline_model_parallel:
        ...
        if self.mps and self.data_parallel_world_size > 1:
            self._grad_norm_buf = torch.mps.DoubleTensor(self.data_parallel_world_size)
        else:
        ...
        # get detailed mps environment
        if self.mps:
            self.mps_env = utils.mpsEnvironment()
            if self.data_parallel_world_size > 1:
                self.mps_env_arr = distributed_utils.all_gather_list(
                    self.mps_env, group=distributed_utils.get_global_group()
                )
            else:
                self.mps_env_arr = [self.mps_env]
            if self.data_parallel_rank == 0:
                utils.mpsEnvironment.pretty_print_mps_env_list(self.mps_env_arr)
        else:
            self.mps_env = None
            self.mps_env_arr = None
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                ...
                # emptying the mps cache after the first step can
                # reduce the chance of OOM
                if self.mps and self.get_num_updates() == 0:
        ...
        try:
            ...
            grad_norm = torch.tensor(0.0).to(torch.device("mps"))
            self.zero_grad()
        except RuntimeError as e:
            if "out of memory" in str(e):
                self._log_oom(e)
        ...
        if not overflow or self.cfg.distributed_training.ddp_backend == "slowmo":
            ...
            if self.tpu:
                ...
                if self.mps and self.mps_env is not None:
                    pass
                    # log minimum free memory over the iteration
                    # gb_used = torch.mps.current_allocated_memory() / 1024 / 1024 / 1024
                    # torch.mps.reset_peak_memory_stats()
                    # gb_free = self.mps_env.total_memory_in_GB - gb_used
                    # metrics.log_scalar(
                    #     "gb_free", gb_free, priority=1500, round=1, weight=0
                    # )
                ...
                # clear mps cache to reduce memory fragmentation
                if (
                    self.mps
    ...
    def clip_grad_norm(self, clip_norm):
        def agg_norm_fn(total_norm):
            total_norm = total_norm.to(torch.device("mps")).float() ** 2
    ...
    def _prepare_sample(self, sample, is_dummy=False):
        ...
        if self.mps:
            if self.pipeline_model_parallel:
                if "target" in sample:
                    sample["target"] = utils.move_to_mps(
            ...
            sample = utils.move_to_mps(sample, device=self.device)
    ...
    def _log_oom(self, exc):
        ...
        logger.warning(msg)
        if torch.backends.mps.is_available() and hasattr(torch.mps, "memory_summary"):
            for device_idx in range(torch.backends.mps.is_available()):
                logger.warning(torch.mps.memory_summary(device=device_idx))
        sys.stderr.flush()

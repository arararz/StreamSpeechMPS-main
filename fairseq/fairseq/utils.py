...
def move_to_mps(sample, device=None):
    device = device or 0

    def _move_to_mps(tensor):
        # non_blocking is ignored if tensor is not pinned, so we can always set
        # to True (see github.com/PyTorchLightning/pytorch-lightning/issues/620)
        return tensor.to(device=torch.device("mps"), non_blocking=True)
...
class mpsEnvironment(object):
    def __init__(self):
        cur_device = 0
        # prop = torch.mps.get_device_properties("mps:{}".format(cur_device))
        # self.name = prop.name
    ...
    @staticmethod
    def pretty_print_mps_env_list(mps_env_list):
        """
        Given a list of mpsEnviorments, pretty print them
        """
        # num_workers = len(mps_env_list)
        # center = "mps enviroments for all {} workers".format(num_workers)
        # banner_len = 40 - len(center) // 2
        # first_line = "*" * banner_len + center + "*" * banner_len
        # logger.info(first_line)
        # for r, env in enumerate(mps_env_list):
        #     logger.info(
        #         "rank {:3d}: ".format(r)
...
def hotreload_function(name=None):
    ...
    * Requirements:
        ...
        2. set environment HOTRELOAD_PAUSE=1 mps_LAUNCH_BLOCKING=1
    ...
    * How to use:
        ...
        2. in bash run scripts:
            ...
            export mps_VISIBLE_DEVICES=0 # single-gpu
            HOTRELOAD_PAUSE=1 mps_LAUNCH_BLOCKING=1 python -m jurigged -w ${watch_dir} --poll 2 -v train.py ......
            ```
    * NOTE:
    ...
    * Limitation:
    ...
    def hotreload_decorator(func):
        ...
        cublk = bool(os.environ.get("mps_LAUNCH_BLOCKING", 0))

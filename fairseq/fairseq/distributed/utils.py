...
# Whether to use XLA ops (e.g., on TPUs) instead of mps ops.
_USE_XLA = False
    ...
    if cfg.pipeline_model_parallel:
        ...
        with open_dict(cfg):
            cfg.distributed_num_procs = min(
                torch.backends.mps.is_available(), cfg.distributed_world_size
            )
    else:
        if cfg.device_id > 0:
            logger.info(
                "setting mps device={} on rank {}".format(
                    cfg.device_id, cfg.distributed_rank
                )
            )
            pass  # torch.mps.set_device(cfg.device_id)
...
def _infer_torch_distributed_launch_init(cfg: DistributedTrainingConfig):
    ...
    cfg.device_id = cfg.distributed_rank % torch.backends.mps.is_available()
    ...
    if node_list is not None:
        try:
            ...
            if ntasks_per_node == 1:
                gpus_per_node = torch.backends.mps.is_available()
...
def _infer_single_node_init(cfg: DistributedTrainingConfig):
    assert (
        cfg.distributed_world_size <= torch.backends.mps.is_available()
    ), f"world size is {cfg.distributed_world_size} but have {torch.backends.mps.is_available()} available devices"
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            if torch.backends.mps.is_available():
                dist.all_reduce(torch.zeros(1).to(torch.device("mps")))
    ...
    if cfg.common.model_parallel_size > 1:
        try:
            from fairseq.model_parallel.megatron.mpu import (
                ...
                model_parallel_mps_manual_seed,
            )
        except ImportError:
            raise ImportError(
        ...
        model_parallel_mps_manual_seed(cfg.common.seed)
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    ...
    if torch.backends.mps.is_available() and not cfg.common.cpu and not cfg.common.tpu:
        pass  # torch.mps.set_device(cfg.distributed_training.device_id)
...
def all_gather_list(data, group=None, max_size=16384):
    ...
    data. Note that *data* must be picklable and any mps tensors will be moved
    ...
    if (
        ...
        all_gather_list._buffer = torch.mps.ByteTensor(buffer_size)

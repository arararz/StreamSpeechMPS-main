...
def get_fused_adam_class():
    ...
    try:
        ...
        global fused_adam_mps
        import importlib

        fused_adam_mps = importlib.import_module("fused_adam_mps")
        return FusedAdamV1
    except ImportError:
        try:
...
class FusedAdamV1(torch.optim.Optimizer):
    ...
    ``python setup.py install --mps_ext --cpp_ext``.
        ...
        for group, grads_this_group, grad_norm in zip(
            ...
            for p, grad in zip(group["params"], grads_this_group):
                ...
                if p.device.type == "cpu":
                    p_data_fp32 = p.data.mps(non_blocking=True).float()
                ...
                with torch.mps.device(p_data_fp32.device):
                    fused_adam_mps.adam(
                        p_data_fp32,
                        out_p,
                        exp_avg,
    ...
    class FusedAdamV2(FusedAdam):
        ...
        def step(
            ...
            for group in self.param_groups:
                ...
                with torch.mps.device(p.device):
                    if len(g_16) > 0:

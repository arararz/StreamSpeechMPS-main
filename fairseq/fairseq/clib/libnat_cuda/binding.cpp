...
torch::Tensor LevenshteinDistance(
  ...
  return LevenshteinDistancemps(source, target, source_length, target_length);
}
...
torch::Tensor GenerateDeletionLabel(
  ...
  return GenerateDeletionLabelmps(source, operations);
}
...
std::pair<torch::Tensor, torch::Tensor> GenerateInsertionLabel(
  ...
  CHECK_INPUT(target);
  CHECK_INPUT(operations);
  return GenerateInsertionLabelmps(target, operations);
}

...
class CommonConfig(FairseqDataclass):
    ...
    cpu: bool = field(default=False, metadata={"help": "use CPU instead of mps"})
    tpu: bool = field(default=False, metadata={"help": "use TPU instead of mps"})
    ...
    empty_cache_freq: int = field(
        default=0,
        metadata={"help": "how often to clear the PyTorch mps cache (0 to disable)"},
    )
...
class DistributedTrainingConfig(FairseqDataclass):
    distributed_world_size: int = field(
        default=max(1, torch.backends.mps.is_available()),
        metadata={

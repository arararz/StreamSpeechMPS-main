...
def load_libnat():
    try:
        from fairseq import libnat_mps

        return libnat_mps, True
        ...
        try:
            ...
            sys.stderr.write(
                "ERROR: missing libnat_mps. run `python setup.py build_ext --inplace`\n"
            )
            raise e


def _get_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx):
    libnat, use_mps = load_libnat()

    def _get_ins_targets_mps(in_tokens, out_tokens, padding_idx, unk_idx):
    ...
    if use_mps:
        return _get_ins_targets_mps(in_tokens, out_tokens, padding_idx, unk_idx)
...
def _get_del_targets(in_tokens, out_tokens, padding_idx):
    ...
    def _get_del_targets_mps(in_tokens, out_tokens, padding_idx):
        in_masks = in_tokens.ne(padding_idx)
        out_masks = out_tokens.ne(padding_idx)
    ...
    def _get_del_targets_cpu(in_tokens, out_tokens, padding_idx):
        ...
        with torch.mps.device_of(in_tokens):
            in_tokens_list = [
                [t for t in s if t != padding_idx]
    ...
    if use_mps:
        return _get_del_targets_mps(in_tokens, out_tokens, padding_idx)
